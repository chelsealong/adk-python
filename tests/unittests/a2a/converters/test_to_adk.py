...
class TestToAdk:
  ...
  def test_convert_a2a_task_to_event_working_with_tool_activity_not_final(
      self,
  ):
    """A non-terminal (working) task with tool activity is not final."""
    a2a_part = _make_a2a_part_for_test({})
    task = Task(
        id="task-1",
        status=_compat.make_task_status(
    ...
    mock_part_converter = Mock(return_value=[function_call_part])
    ...
    assert event is not None
    assert event.get_function_calls()
    assert not event.actions.skip_summarization
