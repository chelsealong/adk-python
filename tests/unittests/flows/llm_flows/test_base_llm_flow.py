...
async def test_run_live_transfers_when_transfer_response_is_not_first_part():
  """Test that run_live transfers even if transfer_to_agent's function
  response is not content.parts[0]."""
  ...
  # Mock _receive_from_model to yield an event whose transfer_to_agent
  # function response is the second part, not the first.
  other_part = types.Part(
      function_response=types.FunctionResponse(name='some_other_tool')
  )
  transfer_part = types.Part(
      function_response=types.FunctionResponse(name='transfer_to_agent')
  )
  content = types.Content(parts=[other_part, transfer_part])
  transfer_event = Event(
      id=Event.new_id(),
      invocation_id=invocation_context.invocation_id,
      author=agent.name,
  ...
  # Mock _get_agent_to_run to return a mock agent and track whether the
  # sub-agent was actually run (i.e. the transfer happened).
  sub_agent_run = False
  mock_sub_agent = mock.Mock()
  mock_sub_agent.run_live = mock.Mock()

  async def mock_run_live_sub_agent(child_ctx, *args, **kwargs):
    nonlocal sub_agent_run
    sub_agent_run = True
    for item in []:
      yield item
  ...
  assert sub_agent_run, (
      'Expected the flow to transfer to the sub-agent even though the'
      " transfer_to_agent function response wasn't content.parts[0]."
  )
